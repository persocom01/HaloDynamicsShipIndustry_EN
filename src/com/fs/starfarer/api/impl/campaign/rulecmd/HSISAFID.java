...
public class HSISAFID extends FleetInteractionDialogPluginImpl {
    protected HSISAFEContext context = new HSISAFEContext();
    protected boolean recoveredCrew = true;
...
//	}
	...
	public HSISAFID(FIDConfig params) {
		this.config = params;
...
//			break;
	...
	protected boolean okToLeave = true;
...
//			winningPath();
		if (context.didPlayerWinMostRecentBattleOfEncounter() ||
				(context.getLastEngagementOutcome() == EngagementOutcome.BATTLE_PLAYER_WIN)||checkEnmeyEngageable(context)) {
			winningPath();
		} else {
			losingPath();
		}
	...
	protected boolean checkEnmeyEngageable(FleetEncounterContext context){
		...
		for(FleetMemberAPI member:data.getMembersListCopy()){
			if(member.canBeDeployedForCombat()&&context.getDataFor(fleet).getDeployedInLastEngagement().contains(member)){
				return true;
			}
		}
		return false;
	...
	protected void winningPath() {
        options.clearOptions();
        DataForEncounterSide playerData = context.getDataFor(Global.getSector().getPlayerFleet());
        context.getDataFor(otherFleet).setDisengaged(true);

        if (context.getBattle() != null && context.getBattle().getNonPlayerSideSnapshot() != null) {
        ...
        if (!recoveredCrew) {
            recoveredCrew = true;
            if ((int) playerData.getRecoverableCrewLosses().getCrewInt()
                    + (int) playerData.getRecoverableCrewLosses().getMarines() > 0) {
                addText(getString("recoveryReport"));
                DataForEncounterSide data = context.getDataFor(playerFleet);
                int crewRecovered = (int) data.getCrewLossesDuringLastEngagement().getCrew();
                int marinesRecovered = (int) data.getCrewLossesDuringLastEngagement().getMarines();
        ...
        if (!didRecoveryCheck) {
            ...
            if (recoverableShips == null)
                recoverableShips = new ArrayList<FleetMemberAPI>();
            if (storyRecoverableShips == null)
                storyRecoverableShips = new ArrayList<FleetMemberAPI>();
            ...
            if (!recoverableShips.isEmpty() || !storyRecoverableShips.isEmpty()) {
                ...
                recoveredShips.clear();
                recoveredShips.addAll(recoverableShips);
                FleetEncounterContext.recoverShips(recoverableShips, context, playerFleet, otherFleet);

                ListenerUtil.reportShipsRecovered(recoverableShips, dialog);
        ...
        // "noSalvageReportPlayerDidNothing":"Your $fleetOrShip does not participate in
        // salvage operations due to its limited contributions throughout the
        // encounter.",
        // "salvageReportPlayer":"Your $fleetOrShip is able to participate in salvage
        // operations due to its contributions throughout the encounter.",
