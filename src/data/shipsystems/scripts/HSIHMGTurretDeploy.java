...
public class HSIHMGTurretDeploy extends BaseShipSystemScript implements MineStrikeStatsAIInfoProvider {
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (state == State.IN) {
			...
			if (target != null) {
				...
				if (target != null) {
					spawnTurret(ship, target,"heavymg");
				}
			}
	...
	public void spawnTurret(ShipAPI source, Vector2f mineLoc,String weaponid) {
		...
		ShipVariantAPI variant = Global.getSettings().getVariant("HSI_PhaseTurret_HMGTurret");
		variant.clearSlot("WS0001");
		variant.addWeapon("WS0001", weaponid);
		ShipAPI turret = engine.createFXDrone(variant);
		turret.setCollisionClass(CollisionClass.SHIP);
		turret.setOwner(source.getOwner());
		turret.setDrone(false);
		turret.setHullSize(HullSize.FRIGATE);
		engine.addEntity(turret);
		turret.getLocation().set(currLoc);
		MutableShipStatsAPI turretStats = turret.getMutableStats();
		...
		if (turret != null && turret.getVariant() != null) {
			carrier = turret.getVariant().isCarrier() && !turret.getVariant().isCombat();
		}
		if (carrier) {
		...
		turret.setShipAI(Global.getSettings().createDefaultShipAI(turret,config));
		turret.setHullSize(HullSize.FIGHTER);

		Global.getCombatEngine().addPlugin(PhaseTurretStats(turret, side));

		// mine.setFlightTime((float) Math.random());
		// liveTime = 0.01f;

		Global.getSoundPlayer().playSound("mine_teleport", 1f, 1f, turret.getLocation(), turret.getVelocity());
	}

	protected EveryFrameCombatPlugin PhaseTurretStats(final ShipAPI mine, final int owner) {
		return new BaseEveryFrameCombatPlugin() {
			FaderUtil fader = new FaderUtil(0, 0.5f, 0.5f, false, false);
			IntervalUtil maxFlightTime = new IntervalUtil(90f, 90f);
			...
			public void advance(float amount, List<InputEventAPI> events) {
				...
				float checkrange = mine.getAllWeapons().get(0).getRange()/1.5f;
				...
				if(!hasEnemyAround) fader.fadeOut();
				...
				if(fader.getBrightness()<1){
					if(!mine.getAllWeapons().isEmpty())
					mine.getAllWeapons().get(0).disable();
				}else{
					if(!mine.getAllWeapons().isEmpty())
					mine.getAllWeapons().get(0).repair();
				}
